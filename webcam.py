...
def capture_loop(quit_flag):
    ...
    cap = cv2.Videocapter(0) # get file descriptor for webcam
...
def main_loop():
    ...
    if __name__ == "__main__":
        ...
        quit_loop = Process(target = quit_loop, args = [quit_flag]).start()
        capture_loop = Process(target = capture_loop, args = [quit_flag]).start()
